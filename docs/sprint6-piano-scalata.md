...
> - **A2 CHIUSA il 2026-07-16 (round 3, n=61)** → **NON CONCLUSIVO** per il terzo round consecutivo; bleed long normalizzato in modo monotòno (-0.134R → -0.065R → **-0.040R** reale). Esito: nessun bleed long robusto, **long tenuto a rischio pieno**. Tappa chiusa su decisione utente (niente quarto round), trigger disattivato in `jobs/sprint6_agenda.py`. Storico dei tre round in `docs/sprint6-long-gate.md`, script `jobs/long_gate_analysis.py`.
